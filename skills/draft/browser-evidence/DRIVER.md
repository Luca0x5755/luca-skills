...
- **`flush_case`** — writes `network.json`, `console.json`, `manifest.json` (with the case's `channel`) and the report skeleton: fields as given (missing ones as 「未提供」), one heading per step with its shots and `evidence` payloads embedded (fenced block, excerpted past 1500 chars, raw file linked), the recorded 機器事實 lines — and the agent's half left blank: an **觀察** line per step, one case-level **對照** line. A report exists for every captured case by construction, not by memory. The skeleton landing on disk is also the hand-off: from that moment the case's 觀察 and 對照 lines belong to the agent, while the script drives on.
...
- **`verify_run`** — the staging gate's number: recounts blank 觀察 and 對照 lines across the run's case reports, rewrites the count line and each case link's 對照 tag in the run index, and prints the count. Re-runnable, so the index stays honest after the 觀察 pass; only 0 clears staging.
